...
int parser_read_file(
  ...
  lasagna_open(inputfile,filename,"r",errmsg);

  counter = 0;
  while (fgets(line,_LINE_LENGTH_MAX_,inputfile) != NULL) {
    lasagna_call(parser_read_line(line,&is_data,name,value,errmsg),errmsg,errmsg);
    if (is_data == _TRUE_) counter++;
  }

  lasagna_test(counter == 0,
	     errmsg,
	     "No readable input in file %s",filename);

  lasagna_alloc(pfc->filename,(strlen(filename)+1)*sizeof(char),errmsg);
  strcpy(pfc->filename,filename);

  lasagna_call(parser_init(pfc,counter,errmsg),
	     errmsg,
	     errmsg);
...
int parser_init(
  ...
  if (size > 0) {
    pfc->size=size;
    lasagna_alloc(pfc->name,size*sizeof(FileArg),errmsg);
    lasagna_alloc(pfc->value,size*sizeof(FileArg),errmsg);
    lasagna_alloc(pfc->read,size*sizeof(short),errmsg);
  }
...
int parser_read_line(
  ...
  lasagna_test(right-left+1 >= _ARGUMENT_LENGTH_MAX_,
	     errmsg,
...
int parser_read_int(
  ...
  * found = _FALSE_;
  ...
  lasagna_test(sscanf(pfc->value[index],"%d",value) != 1,
	     errmsg,
  ...
  * found = _TRUE_;
  ...
  for (i=index+1; i < pfc->size; i++) {
    lasagna_test(strcmp(pfc->name[i],name) == 0,
	       errmsg,
...
int parser_read_double(
  ...
  * found = _FALSE_;
  ...
  lasagna_test(sscanf(pfc->value[index],"%lg",value) != 1,
	     errmsg,
	     "could not read value of parameter %s in file %s\n",name,pfc->filename);
...
int parser_read_list_of_doubles(
  ...
  *size = i; 
  ...
  lasagna_alloc(list,*size*sizeof(double),errmsg);
  *pointer_to_list = list;
  ...
  do {
    ...
    lasagna_test(sscanf(string_with_one_value,"%lg",&(list[i-1])) != 1,
	       errmsg,
...
int parser_read_list_of_integers(
  ...
  *size = i; 
  ...
  /* free and re-allocate array of values */
  lasagna_alloc(list,*size*sizeof(int),errmsg);
  *pointer_to_list = list;
  ...
  do {
    ...
    lasagna_test(sscanf(string_with_one_value,"%d",&(list[i-1])) != 1,
	       errmsg,
	       "could not read %dth value of list of parameters %s in file %s\n",
	       i,
	       name,
...
int parser_read_list_of_strings(
  ...
  *size = i; 
  ...
  /* free and re-allocate array of values */
  lasagna_alloc(list,*size*sizeof(FileArg),errmsg);
  *pointer_to_list = list;
...
int parser_cat(
  ...
  lasagna_test(pfc1->size < 0.,
	     errmsg,
	     "size of file_content structure probably not initialized properly\n");

  lasagna_test(pfc2->size < 0.,
	     errmsg,
	     "size of file_content structure probably not initialized properly\n");

  if (pfc1->size == 0) {
    lasagna_alloc(pfc3->filename,(strlen(pfc2->filename)+1)*sizeof(char),errmsg);
    sprintf(pfc3->filename,"%s",pfc2->filename);
  }
  if (pfc2->size == 0) {
    lasagna_alloc(pfc3->filename,(strlen(pfc1->filename)+1)*sizeof(char),errmsg);
    sprintf(pfc3->filename,"%s",pfc1->filename);
  }
  if ((pfc1->size !=0) && (pfc2->size != 0)) {
    lasagna_alloc(pfc3->filename,(strlen(pfc1->filename)+strlen(pfc2->filename)+5)*sizeof(char),errmsg);
    sprintf(pfc3->filename,"%s or %s",pfc1->filename,pfc2->filename);
  }

  pfc3->size = pfc1->size + pfc2->size;
  lasagna_alloc(pfc3->value,pfc3->size*sizeof(FileArg),errmsg);
  lasagna_alloc(pfc3->name,pfc3->size*sizeof(FileArg),errmsg);
  lasagna_alloc(pfc3->read,pfc3->size*sizeof(short),errmsg);
